...
const Config = Schema.object({
	monitors: Schema.array(Schema.object({
		id: Schema.string(),
		url: Schema.string(),
		healthyCode: Schema.number().default(200),
		fixable: Schema.boolean().default(false)
	})).default([]),
	intervalMs: Schema.number().min(0).default(0),
	subagentProvider: Schema.string().default("spawn"),
	surgicalConstraints: Schema.array(Schema.string()).default([]),
	seedSessionId: Schema.string().default("")
});
/**
* Probe a monitor target; the happy-path gate is HTTP reachable + healthy code.
* `fetchImpl` is injectable for tests; defaults to the global fetch.
*/
async function probe(monitor, fetchImpl = fetch) {
	try {
...
function heuristicDecide(healthy) {
	return healthy ? {
		needs_fix: false,
		confidence: 1
	} : {
...
*/
function evaluate(probeAfter) {
	if (probeAfter.healthy) return {
		passed: true,
		reason: "target healthy after fix"
	};
	return {
		passed: false,
		reason: `target still unhealthy (status ${probeAfter.status})`
	};
}
/** Build the fix prompt from a decision and the surgical constraints. */
function buildFixPrompt(decision, constraints) {
	return [{
		type: "text",
...
*/
async function resolveParent(ctx, seedSessionId) {
	if (seedSessionId) {
		const bySeed = ctx.agents.get(seedSessionId);
...
function apply(ctx, config) {
	...
	async function runCycle(monitor) {
		...
		const result = await (await ctx.subagents.start(provider, request)).result;
		return {
			monitor: monitor.id,
			healthy: probeResult.healthy,
			decision,
	...
	ctx.provide("waom", {
		run: (monitor) => runCycle(monitor),
		monitors: () => config.monitors
	});
	if (intervalMs > 0) {
		const timer = setInterval(() => {
			ctx.logger?.info?.("waom: tick — host-level autonomous scheduling deferred");
		}, intervalMs);
		ctx.effect(() => () => {
			clearInterval(timer);
		}, "waom.ticker()");
...
//#endregion
export { Config, apply, buildFixPrompt, evaluate, heuristicDecide, inject, name, probe, resolveParent };
